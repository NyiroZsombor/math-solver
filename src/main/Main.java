...
public class Main {
    ...
    public static void input() {
        ...
        System.out.println(Lexer.toString(preParser.getTokens()));
    
        scanner.close();
    }

    // FIX (1+1)*2: MULT or DIV
    ...
    public static void test() {
        ...
        */
        tests[0] = "((4+3*2)-5)/1+2";
        tests[1] = "12/((3+1)*2)+(5-3*2)";
        tests[2] = "9-(2*(3+4))/(2+8)";
        tests[3] = "6+(7*2-(5/1))-3*2";
        tests[4] = "8/2+3*4-(6+1)";

        for (String test : tests) {
            System.out.println(Lexer.toStringTokens(new PreParser(new Lexer(test).getTokens()).getTokens()));
            System.out.println();
        }
    }
}